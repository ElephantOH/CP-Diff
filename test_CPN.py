...
def get_cell_inputs(cell_input, snn_result, source, device):
    inputs_x0 = []
    inputs_xt = []
...
def sample_by_snn(snn, model, source, device):
    sample_sequence = topological_sort(snn)
    snn_result = {key: None for key in snn}
    with torch.no_grad():
        for snn_id in sample_sequence:
            cell_info = snn[snn_id] # id : [time_layer, [input_ids], intensity, capacity]
            input_x0, input_xt = get_cell_inputs(cell_info[1], snn_result, source, device)
            if snn_id == 0.0:
...
def sample_and_test(args):
    ...
    snn_name = "snn_" + args.snn_name
    snn = getattr(args, snn_name)
    ...
    save_dir = args.checkpoint_path + "/generated_samples/SNN({})".format(args.which_epoch)
    ...
    for iteration, (source_data, target_data, _, _) in progress_bar:
        ...
        if args.input_channels == 3:
            ...
            source_data = source_data.squeeze(1)
        fake_sample = sample_by_snn(snn, model, source_data, device)
