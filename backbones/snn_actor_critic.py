import threading
import torch
from .snn_replayer import ReplayBuffer
from .snn_searcher import SNNACSearcher
...
class SNNACTrainer:
    def __init__(self, args, snn_dim=6, hidden_dim=256, z_dim=100, env_step=30000,
                       snn_lr=1e-4, snn_lrf=1e-5, batch_size=16,
                 snn=None, model=None, dataloader=None, device=torch.device('cuda:0')):
        self.args = args
        self.device = device
        ...
        self.z_dim = z_dim
        self.actor = Actor(state_dim=snn_dim, hidden_dim=hidden_dim, z_dim=z_dim, dtype=self.dtype).to(self.device)
        self.critic = Critic(state_dim=snn_dim, hidden_dim=hidden_dim, z_dim=z_dim, dtype=self.dtype).to(self.device)

        self.actor_optimizer = optim.Adam(self.actor.parameters(), lr=snn_lr, betas=(self.args.beta1, self.args.beta2))
        self.critic_optimizer = optim.Adam(self.critic.parameters(), lr=snn_lr, betas=(self.args.beta1, self.args.beta2))
        self.env_steps = env_step
        self.actor_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.actor_optimizer, self.env_steps, eta_min=snn_lrf)
        self.critic_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.critic_optimizer, self.env_steps, eta_min=snn_lrf)
        self.lambda_reg = 0.1
        self.lambda_vm = 1
        ...
        self.org_v = -1000.
        self.args.C = 60
        self.searcher = SNNACSearcher(self.args, self.snn, self.model, self.dataloader)
    ...
    def validate(self):
        with torch.no_grad():
            ...
            latent_z = torch.randn(self.z_dim, dtype=self.dtype, device=self.device)
            new_states = self.actor(states, latent_z)
            snn_test = tensor_to_snn(self.snn, new_states[0])
            v = sample_one(self.args, self.dataloader, self.model, snn_test, self.device)
            return snn_test, v, new_states[0]

    def get_result(self):
        snn_test, v_test, _ = self.validate()
        print("Actor Opt V")
        print("=" * 50)
        print("\033[92m" + str(snn_test) + "\033[0m")
        print("Actor Opt V: " + str(v_test))
        print("=" * 50 + "\n")
        print(" ")
        print("Truth Opt V")
        print("=" * 50)
        print("\033[93m" + str(self.searcher.opt_snn) + "\033[0m")
        print("Truth Opt V: " + str(self.searcher.opt_v))
        print("=" * 50 + "\n")
        return snn_test, v_test, self.searcher.opt_snn, self.opt_v

    def init_snn(self):
        if self.args.snn_init_search:
            self.opt_snn = self.searcher.ternary_search_init_snn(search_round=1)
        else:
    ...
    def train(self):
        ...
        for step in tqdm(range(self.env_steps), desc="Training SNN", ncols=100):
            a_loss, c_loss = self.update_networks()

            if step % int(self.args.snn_log_time / 5) == 0 and step != 0:
                time.sleep(self.args.snn_sleep_time)

            if step % self.args.snn_log_time == 0 and step != 0:
                snn_test, v_test, snn_tensor_test = self.validate()

                if v_test > self.searcher.opt_v:
                    self.searcher.buffer.push(snn_tensor_test, v_test)

                snn_test = '\n'.join([f'{key}: {value}' for key, value in snn_test.items()])
                tqdm.write(f"\n{'-' * 50}\n"
                           f"Step: {step}\n"
